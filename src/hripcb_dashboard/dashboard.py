...
const combinedTechniques = new Set(['gaussian_bbhe', 'wavelet_homomorphic', 'bilateral_agcwd', 'nlm_msr', 'tv_top_black_hat']);
const techniqueLabels = {{'original': 'Original', 'gaussian': 'Gaussian', 'bbhe': 'BBHE', 'gaussian_bbhe': 'Gaussian + BBHE', 'wavelet': 'Wavelet', 'homomorphic': 'Homomorphic', 'wavelet_homomorphic': 'Wavelet + Homomorphic', 'bilateral': 'Bilateral', 'agcwd': 'AGCWD', 'bilateral_agcwd': 'Bilateral + AGCWD', 'nlm': 'Non-local Means', 'msr': 'Multi-Scale Retinex', 'nlm_msr': 'NLM + MSR', 'tv': 'TV', 'top_black_hat': 'Top-hat + Black-hat', 'tv_top_black_hat': 'TV + Top-hat + Black-hat'}};
const techniqueLabel = value => techniqueLabels[String(value || '').toLowerCase()] || String(value || 'unknown').replaceAll('_', ' + ');
const runType = record => record.is_combined !== undefined && record.is_combined !== null ? (record.is_combined ? 'combined' : 'reference') : (record.evaluation_stage === 'combined' || combinedTechniques.has(String(record.technique || '').toLowerCase()) ? 'combined' : 'reference');
const isSharedOriginal = record => record.model_id === 'baseline' && record.split === 'val' && String(record.technique || '').toLowerCase() === 'original' && ['member1', 'member2', 'member3', 'member4', 'member5'].includes(record.module);
...
const displayTechnique = record => isSharedOriginal(record) && state.module === 'all' ? 'Original / Shared baseline control' : techniqueLabel(record.technique);
...
const sortedRecords = () => [...records].sort((a,b) => {{ const difference = state.direction === 'asc' ? metric(a) - metric(b) : metric(b) - metric(a); return difference || String(b.id || '').localeCompare(String(a.id || '')); }});
