...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }} body {{ margin: 0; background: radial-gradient(circle at 8% 0%, rgba(207, 229, 255, .72), transparent 34%), radial-gradient(circle at 92% 10%, rgba(215, 245, 232, .62), transparent 32%), var(--canvas); color: var(--ink); }}
    ...
    .controls {{ display: grid; grid-template-columns: repeat(4, minmax(140px, 1fr)) 1.4fr 1fr 1.6fr; gap: 10px; margin: 18px 0; }}
...
<main>
  ...
  <h1>{escaped_title}</h1>
  <p class="intro">A shared comparison surface for every preprocessing module. Select a row to inspect its preview, parameters, and detector metrics. The primary ranking metric is <strong>{html.escape(primary_metric_label)}</strong>.</p>
  <section class="hero">
...
const state = {{model: 'all', split: 'all', module: 'all', technique: 'all', search: '', sortMetric: primaryMetric, direction: 'desc', selected: null}};
...
const recordsForFields = fields => records.filter(record => fields.every(field => state[field] === 'all' || fieldValue(record, field) === state[field]));
...
const matches = record => {{ const text = JSON.stringify(record).toLowerCase(); return filterFields.every(field => state[field] === 'all' || fieldValue(record, field) === state[field]) && (!state.search || text.includes(state.search)); }};
...
function renderSummary() {{ const ranked = sortedRecords().filter(matches); const best = ranked[0]; const memberModules = new Set(ranked.map(record => record.module).filter(module => module && module !== 'baseline')); document.getElementById('runCount').textContent = ranked.length; document.getElementById('moduleCount').textContent = memberModules.size; document.getElementById('coverageText').textContent = parameterKeys.length + ' parameter dimensions / ' + metricKeys.length + ' metrics · sorted by ' + label(state.sortMetric); if (best) {{ document.getElementById('winnerName').textContent = best.id; document.getElementById('winnerTechnique').textContent = (best.model_label || modelValue(best)) + ' / ' + best.module + ' / ' + best.technique; document.getElementById('winnerScore').textContent = fmt(best.metrics?.[state.sortMetric]); }} }}
...
function filterRows() {{ for (const field of filterFields) state[field] = document.getElementById(field + 'Filter').value; state.search = document.getElementById('searchFilter').value.toLowerCase().trim(); syncCascadedFilters(); renderSummary(); renderRows(); }}
fillSelect('metricOptions', metricKeys); renderHead(); syncCascadedFilters(); renderSummary(); renderRows(); for (const field of filterFields) document.getElementById(field + 'Filter').addEventListener('change', filterRows); document.getElementById('sortMetric').addEventListener('change', sortRows); document.getElementById('sortDirection').addEventListener('change', sortRows); document.getElementById('searchFilter').addEventListener('input', filterRows);
</script>
</body>
</html>
"""
