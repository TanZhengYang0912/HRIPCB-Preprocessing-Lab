#!/usr/bin/env python3
"""Create the official original/preprocessed/retrained test comparison records."""
...
def build(output: Path, baseline_original_metrics: Path, baseline_preprocessed_results: Path, candidate_results: Path | None = None) -> Path:
    original = json.loads(Path(baseline_original_metrics).read_text(encoding="utf-8"))
    baseline_preprocessed = json.loads(Path(baseline_preprocessed_results).read_text(encoding="utf-8"))[0]
    original_metrics = {
    ...
    records = [original_record, baseline_preprocessed]
...
def main() -> int:
    ...
    parser.add_argument("--baseline-preprocessed", type=Path, default=Path("runs/baseline_wavelet_homomorphic_test/results.json"))
    parser.add_argument("--candidate", type=Path, default=None)
    args = parser.parse_args()
    print(f"official test records: {build(args.output, args.baseline_original, args.baseline_preprocessed, args.candidate)}")
    return 0
