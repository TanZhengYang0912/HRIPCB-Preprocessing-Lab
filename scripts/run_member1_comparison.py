...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, default=Path("runs/member1_baseline_comparison"))
    parser.add_argument("--config", type=Path, default=Path("configs/member1.yaml"))
    parser.add_argument("--sample", default=None)
    parser.add_argument("--checkpoint", type=Path, default=None)
    parser.add_argument("--no-model-eval", action="store_true")
...
def main() -> int:
    ...
    output = run_comparison(
        ...
        evaluate_model=not args.no_model_eval,
    )
